import { useGetProductsQuery, useGetCategoryQuery } from "@/redux/query/productApi";
      ...
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div className="max-w-xl">
             ...
             <h1 className="text-5xl md:text-7xl font-black italic tracking-tighter mb-4">The Collection.</h1>
             <p className="text-muted-foreground text-lg">
               Browse our curated selection of premium products, designed for those who value quality and style.
             </p>
          </div>
          
          <div className="flex gap-2 w-full md:w-auto">
             <Button variant="outline" rounded-full px-6 h-12 flex gap-2 font-bold group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <aside className="hidden lg:block lg:col-span-3 space-y-10">
             <div>
                <h3 className="text-sm font-black uppercase tracking-widest mb-6 border-l-4 border-primary pl-4">Categories</h3>
                <ul className="space-y-4">
                  <li>
                    <button className="text-foreground font-bold hover:text-primary transition-colors flex justify-between w-full group">
                       All Collections <span className="text-muted-foreground font-medium group-hover:text-primary transition-colors">{productsData?.products?.length || 0}</span>
                    </button>
                  </li>
                  {categories?.map((cat: any) => (
                    <li key={cat.id}>
                      <button className="text-muted-foreground font-bold hover:text-primary transition-colors flex justify-between w-full group">
                         {cat.categoryName} <span className="font-medium group-hover:text-primary transition-colors">12</span>
                      </button>
                    </li>
                  ))}
                </ul>
             ...
             <div>
                <h3 className="text-sm font-black uppercase tracking-widest mb-6 border-l-4 border-primary pl-4">Price Range</h3>
             ...
             <div className="p-8 rounded-[2rem] bg-secondary/30 border border-border/50 overflow-hidden relative group">
                ...
                <Button variant="outline" className="w-full rounded-xl font-bold bg-background">Contact Us</Button>
             </div>
          </aside>
              ...
              <div className="mt-20 flex justify-center gap-2">
                 {[1, 2, 3].map((i) => (
                   <Button 
                    ...
                    className={cn("h-12 w-12 rounded-xl font-bold", i === 1 && "shadow-lg shadow-primary/20")}
                   >
                     {i}
                   </Button>
                 ))}
                 <Button variant="outline" className="h-12 px-6 rounded-xl font-bold">Next</Button>
              </div>
            )}
          </div>
        </div>
